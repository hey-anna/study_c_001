...
// 2씩 커져서 , 다음줄에서 2씩 더해줌
// 총 5층
// 0(i) 일 경우 j는 0123 (5층 -1) 4층까지 반복된다
// 1(i) 일 경우 j는 123
// 2(i) 일 경우 j는 23
// 0(i) * 2 + 1 = 1
// 1(i) * 2 + 1 = 3
// 2(i) * 2 + 1 = 5
// 나는 더하기 1을 뺏지만, k의 0은 해당 안되서 출력이 안되고 1부터 출력이 되는 것 ?
// 그러면 k의 시작하는 숫자를1로 하면 안되나 ? 출력이 안되네,,

// 한번 돌아갈 때 마다 k는 0부터 시작하기때문에, 조건의 i * 2 에 해당하는 숫자 만큼 반복되고, 해당되는 것의 갯수만큼 출력 된다.
// i가 0일 때 --- k는 0이 같기때문에 별 1개 출력 (0(i) * 2)
// i가 1일 때 --- k의 012는 같거나 작기때문에 별 3개 출력 (1(i) * 2)
// i가 2일 때 --- k의 01234 같거나 작기때문에 별 5개 출력 (2(i) * 2)
// 나는 작거나 같으면 별출력 이지만  - 샘이 한 것은 작으면 별출력

// 내방식 대로 해서 근데, k를 삭제 했더니 무한렌더링 됨, 샘방식대로 하자
    return 0;
}